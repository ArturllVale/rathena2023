...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case MS_BASH:
			...
			//skillratio += 30 * skill_lv * sstatus->str/6;
			break;
		case SM_MAGNUM:
		case MS_MAGNUM:
			if(wd->miscflag == 1)
				skillratio += 50 * skill_lv; //Inner 3x3 circle takes 100%+20%*level damage [Playtester]
			else
				skillratio += 30 * skill_lv; //Outer 5x5 circle takes 100%+10%*level damage [Playtester]
			break;
		case MC_MAMMONITE:
		case BS_HAMMERFALL:
		...
		case ML_BRANDISH:
			{
				int ratio = 90 + 20 * skill_lv + sstatus->str * 3;
		...
		case MC_CARTREVOLUTION:
			...
			if(sd && sd->cart_weight)
				...
				skillratio += 200; //Max damage for non players.
			break;
		...
		case ASC_BREAKER:
			...
			skillratio += -100 + 150 * skill_lv + sstatus->str + sstatus->int_;
#endif
			break;
		case PA_SACRIFICE:
		...
		case SU_PICKYPECK_DOUBLE_ATK:
			//skillratio += 100 + 100 * skill_lv;
			skillratio += 70 * skill_lv + sstatus->dex * 5;
		...
		case SU_LUNATICCARROTBEAT2:
			...
			skillratio += 90 * skill_lv + sstatus->dex * 5; // !TODO: What's the STR bonus?
			//RE_LVL_DMOD(100);
			if (sd && pc_checkskill(sd, SU_SPIRITOFLIFE))
				skillratio += skillratio * status_get_hp(src) / status_get_max_hp(src);
			break;
...
#else
			ATK_ADD(wd->damage, wd->damage2, 500 * sc->getSCE(SC_AURABLADE)->val1);
#endif
		}
	}
...
#define MATK_ADD(a) { ad.damage += a; }
		...
		switch (skill_id) {
			...
			case AB_HIGHNESSHEAL:
				ad.damage = 10 * skill_calc_heal(src, target, skill_id, skill_lv, false);
				break;
			case PR_ASPERSIO:
				ad.damage = 40;
				break;
			...
			default: {
				...
				switch(skill_id) {
					...
					case WZ_FIREPILLAR:
						...
						skillratio += -60 + 60 * skill_lv; //20% MATK each hit
						break;
					case WZ_SIGHTRASHER:
					...
					case SL_STIN: //Target size must be small (0) for full damage
						skillratio += (tstatus->size != SZ_SMALL ? -99 : 100 * skill_lv);
						break;
					case SL_STUN:
						skillratio += 120 * skill_lv;
						break;
					...
					case SU_CN_METEOR:
					case SU_CN_METEOR2:
						skillratio += 50 * skill_lv + sstatus->int_ * 5; // !TODO: Confirm INT bonus
						//RE_LVL_DMOD(100);
						break;
					case NPC_VENOMFOG:
						skillratio += 600 + 100 * skill_lv;
		...
		switch(skill_id) {
			...
			case NPC_GRANDDARKNESS: {
					...
					ad.damage = battle_attr_fix(src, target, wd.damage + ad.damage, s_ele, tstatus->def_ele, tstatus->ele_lv) * (100 + 100 * skill_lv) / 100;
					if(src == target) {
						if(src->type == BL_PC)
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int mflag)
{
	...
	switch (skill_id) {
		...
		case HT_LANDMINE:
		case MA_LANDMINE:
			md.damage = skill_lv * (sstatus->dex + 1000) * (100 + sstatus->agi) / 100;
			break;
		case HT_BLASTMINE:
			md.damage = skill_lv * (sstatus->dex / 2 + 1000) * (100 + sstatus->agi) / 100;
			break;
		case HT_CLAYMORETRAP:
			md.damage = skill_lv * (sstatus->dex / 2 + 1000) * (100 + sstatus->agi) / 100;
			break;
		...
		case SN_FALCONASSAULT:
			{
				...
				RE_LVL_MDMOD(100);
#else
				md.damage = 100 + (skill_lv * 6) + (sstatus->dex * 8);
		...
		case HW_GRAVITATION:
			...
			break;
		case PA_PRESSURE:
			md.damage = (500 + 300 + sstatus->str * skill_lv) * 10;
			break;
#endif
		case PA_GOSPEL:
			if (mflag > 0)
